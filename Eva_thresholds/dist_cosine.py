...
for key,value in data.items():
    ...
    for image_path_non in list_image_nonmatch:
        ...
        if (feature1 is None):
            continue
        dist = dist_cosine(vec_image_main,feature1)
    ...
    for image_path_match in list_image_match:
        ...
        if (feature2 is None):
            continue
        dist_ = dist_cosine(vec_image_main,feature2)
        list_dist.append(dist_)
        list_match_label.append(1)

with open("dist_cosine.pkl","wb") as dis:
    pickle.dump(list_dist,dis)
with open("label_matching_cosine.pkl","wb") as label:
